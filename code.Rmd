...
```{r arrivals visualization, echo=F, fig.cap="Total number of tourist arrivals", fig.height=8, fig.width=11}
...
```{r arrival trend, echo=F, fig.cap="Trend of tourists arrivals", fig.height=6, fig.width=7}
...
kable(head(master.data), format = "latex", row.names = F)
kable(tail(master.data), format = "latex", row.names = F)
...
# In this section we will be creating a data frame for a panel model
...
```{r model results, results='asis', echo=F, warning=F}
  texreg(model, caption = "OLS model")
```
