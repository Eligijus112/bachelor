...
For example, the 'head' and the 'tail' of the data which will me analysed looks like this:

```{r, visualising data table, echo=F}

master.data <- plyr::rename(master.data, c('GDP per capita (current US$)' = "GDP.per.capita"))
master.data <- plyr::rename(master.data, c('Population, total' = "Population"))
master.data <- plyr::rename(master.data, c('Working hours' = "Hours.worked"))

kable(head(master.data))
kable(tail(master.data))
...
model <- plm(formula = Total.Arrivals ~ Terror.attacks + Population + GDP.per.capita + Hours.worked, data=master.data, index=c("Country"))
