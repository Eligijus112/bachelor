...
Šio darbo tikslas yra sudaryti turizmo modelį Vakarų Europos šalims bei Amerikai ir Kanadai. Naudojantis viešais pasaulinio banko (World Bank), OECD ir Merilando Universiteto duomenų bazės (the Department of Homeland Security Center of Excellence led by the Maryland University) duomenimis bandėme paaiškinti kasmetinius atvykusių turistų srautų procentinius pokyčius naudodamiesi procentiniais BVP vienam asmeniniui pokyčiais, teroro aktų skaičiaus pokyčiais, mokesčių paslaugoms pokyčiais, kasmetinių dirbamų valandų vidurkių pokyčiais, investicijų į infrastruktūrą pokyčiais bei pačių turistų srautų ankstinių pokyčiais. Ryšio nustatymui naudojome paprastą tiesinį, fiksuotų efektų, atsitiktinių efektų bei dinaminius panelinius modelius. Tokio tipo modelių pasirinkimą lėmė erdvinė blokuotų originalių duomenų struktūra. Išbandę įvairius kintamųjų ankstinius, modelių tipus, pritaikę pažingsninį kintamųjų išmetimo (išmetant mažiausiai reikšmingą) metodą gavome, jog vienintelis kintamasis, darantis reikšmingą įtaką turistų srautų pokyčiams buvo metinis vidutinis pradirbtų valandų skaičiaus pokytis. Tokią silpną koreliaciją tarp endogeninio ir egzogeninių kintamųjų galime paaiškinti tuo, jog vieši duomenys yra per daug agreguoti ir jie stokoja tikslesnio struktūrinio srautų išskaidymo tarp šalių. Pasinaudoję visa išskaidymo informacija, kurią suteikia OECD duomenų bazė ir pasinaudoję tiesiniais modeliais gavome, jog didžiausią įtaką turistų skaičiaus pokyčiams turi šalių, iš kurių atvyksta turistai, BVP vienam asmeniui pokyčiai. BVP vienam asmeniui pokyčiai šalyje, į kurią atvyksta turistai bei visi likę aukščiau minėti kintamieji nėra reikšmingi norint paaiškinti turizmo srautus. Atsižvelgus į literatūrą galime daryti prielaidą, jog naudojantis mokamais duomenimis su detalesne turizmo srautų struktūra galėtume rasti daugiau reikšmingų egzogeninių kintamųjų tarp paminėtų aukščiau.   
...
Wikipedia gives the following definition regarding what is tourism `r Cite(bib=bib, 'tourism.definition', .opts = list(cite.style = "numeric"))`: "Tourism is travel for pleasure; also the theory and practice of touring, the business of attracting, accommodating, and entertaining tourists, and the business of operating tours". According to UNWTO (United Nations World Tourism Organization)`r Citet(bib, 'UNWTO', .opts = list(cite.style = "numeric"))`:
...
In this paper we want to find which economic or social variables significantly influence or have no effect on the tourism flows to help governments in deciding which policies to adapt in order to grow the tourism sector.

\newpage

\section{Countries}

The countries which will be analyzed are all members of the OECD (\textbf{Organisation for Economic Co-operation and Development}) `r Citet(bib, 'OECD', .opts = list(cite.style = "numeric"))`.  

```{r map of OECD countries, echo=F, fig.width=8, fig.height=8, fig.cap= "Map of the countries in this analysis", echo=F, warning=F, include=T, fig.pos="!ht"}
...
mapCountryData(malMap, nameColumnToPlot="OECD", catMethod = "categorical",
  missingCountryCol = gray(.8), oceanCol = 'cyan', mapTitle = "Political map of the world", borderCol = adjustcolor("black", alpha.f = 0.6))
...
```{r arrivals visualization, echo=F, fig.cap="Total number of tourist arrivals", fig.height=8, fig.width=12, fig.pos="!ht", fig.align="center"}
...
In this paper we will focus on the socio - economic ones, because they in general can be expressed in numbers and are not constant over time. 

Out of all of the socio-economic factors, accessibility is one of the most important one. All tourist centers must be easily accessible by various modes of transportation like roads, railways, air and water. To enjoy nature's beautiful sites seeing traveling by roads and railways is a better option. If a tourist plans to reach a remote tour-destination in the quickest possible time, then airway is the most suitable choice and so on. To reflect accessibility we will use the investments to inroad infrastructure ($Investment$) variable.

Another big influencer to tourism are the various attraction sites in a given country: ranging from big ones like amusement parks, natural wonder sites to the small ones like cofferhouses, restaurants. All of these businesses have to maintained and a big factor are the taxes on these services. As a reflection of how governments help or not in increasing the tourism flows to these businesses we will use the taxes on goods and services ($tax$) variable. Also, the overall economic state of a country has a strong correlation with the amount of new shops, hotels and other attractions openings in a given country. To measure the overall state of a country we will use the GDP per capita ($GDP$) variable. Also, a better economic state of a country means that there are more ancillary services: banking and finance, internet and telecom connectivity, hospitals, insurance and so on. 
...
Some economists believe that GDP was the 'Manhattan project of economics' and 'The most significant invention in economics of the twentieth century' `r Cite(bib=bib, 'GDP', .opts = list(cite.style = "numeric"), after=" see pp. 31-32")`. Altough it did help USA and the allies win the Second World War and tackle the bigger crises of the last century, the criticism of this number as a basis around which policies are made is growing. Even the father of GDP, Simon Kuznets, raised concerns about the ever growing popularity of it. One of his most biggest concerns was that GDP as measure of economic well beeing does not take into account the inequality in a society. Becasue of the way national accounts were designed a bigger increase in the spending on luxury goods of the most wealthy people in a society outweight the loss in spending of the most poor of the people in a society. A country in which the rich get richer and poor become poorer may very well have a very high GDP `r Cite(bib=bib, 'GDP', .opts = list(cite.style = "numeric"), after=" see pp. 53")`. But this obviously raises tensions in a society and does not show the true effectivness of an economy. According to Lorenzo Fioramonti: `r Cite(bib=bib, 'opinion', .opts = list(cite.style = "numeric"))`
...
```{r gini vs bvp, echo=F, warning=F ,fig.cap="GDP versus gini", fig.height=4, fig.width=5,  fig.pos="!ht", fig.align="center"}

gini <- read.csv("input/gini.csv", stringsAsFactors = F)
gini <- gini[gini$SUBJECT=="GINI", c("ï..LOCATION", "TIME", "Value")] %>% rename(c("ï..LOCATION"= "Country", "TIME" = "Date", "Value" = "Gini"))

gdp.tot <- read.csv('input/total gdp.csv', stringsAsFactors = F)
gdp.tot <- gdp.tot[gdp.tot$MEASURE=="MLN_USD", c("ï..LOCATION", "TIME", "Value")] %>% rename(c("ï..LOCATION"= "Country", "TIME" = "Date", "Value" = "gdp"))

gdp.tot <- gdp.tot[gdp.tot$Country %in% unique(gini$Country), ]

gini <- ddply(gini, ~Country, function(xframe){
  
  xframe <<- xframe
  xframe$Country <- decoder[decoder$CountryCode==xframe$Country[1], "CountryName"]
  return(xframe)
  
})

gdp.tot <- ddply(gdp.tot, ~Country, function(xframe){
...
gini <- suppressMessages(left_join(gini,gdp.tot))
gini <- gini[gini$Country %in% countries, ]

gini <- gini[, c("Country", "Date", "gdp", "Gini")]

gini$gdp <- log(gini$gdp)
...
modg <- lm(Gini ~ gdp, data=gini)

grid.frame(gini$gdp, gini$Gini,  ylab="Differences in gini", xlab="Differences of log GDP", col=adjustcolor("black", alpha.f = 2), lwd=2)
matplot(gini$gdp, gini$Gini,  lwd=1, lty=1, cex=1.5, pch=20, type="p", col=c('cornflowerblue'), add=T)

```

```{r gini model, results="asis", echo=F, warning=F}
tab <- xtable(summary(modg)$coef, digits=c(3, 3, 3, 3, 3), caption = "Gini = f(gdp)")
print(tab, type="latex", floating = T, only.contents = F, comment = F)
```
As we can see from figure 5 and table 1, there is no strong correlation between the changes in gini coefficient (inequality) and changes in the GDP in the countries which we will analyze. To keep with the trends of contemporary economic analysis we will keep the GDP per capita variable as an indicator for economic state. 
...
```{r visualizing terror, echo=F, fig.cap="Terrorism in OECD countries", fig.height=6, fig.width=8,  fig.pos="!ht", fig.align="center"}
...
```{r visualizing working hours, echo=F, fig.cap="Hours worked in OECD countries", fig.height=6, fig.width=8,  fig.pos="!ht", fig.align="center"}
...
As we can see from figure 7 the overal downwards trend of the working annaul working hours' average is true to the countries which we will analyze.
...
Panel data provides information on individual behaviour, both across individuals and over time - the data has cross-sectional and time-series dimensions. Panel data includes N individuals over T regular time periods. We can denote a panel data model similarly like a typical bivariate reggression model `r Citet(bib, 'econometric.methods', .opts = list(cite.style = "numeric"), after=" , pp. 390")`:
...
The problem with this model is that while giving consistent estimators, the standart errors of those estimators will be understated. Also, OLS is not efficient compared to the generalized least-squares procedure  `r Citet(bib, 'econometric.methods', .opts = list(cite.style = "numeric"), after=" , pp. 391")`.

\newpage

\subsection{Fixed effects model}

In this section we will be creating a fixed effects panel data model  `r Citet(bib, 'econometric.methods', .opts = list(cite.style = "numeric"), after=" , pp. 395 - 399")`. Recall equation (1). The $\alpha_{i}$ is called the \textit{individual effect}.
...
and the \textit{mean absolute percentage error} (MAPE)  `r Citet(bib, 'accuracy', .opts = list(cite.style = "numeric"))`. These statistics are defined by:
...
As we can see from table 6 and the statistics for accuracy table the fixed effects models' predictions are closer to the original values. Note that we have dropped the MAPE statistic because we are measuring percentage changes and the statistic would give misleading results.

\newpage

\subsection{Random effects model}

There are two main types of panel models: the fixed effects model and the random effects models `r Citet(bib, 'econometric.methods', .opts = list(cite.style = "numeric"), after=" , pp. 391 - 393")`. The main difference between them is the assumption of $\alpha_{i}$ regarding the regressor matrix $X_{it}$. In the random effects model case it is assumed that $\alpha_{i}$ and $X_{it}$ do not correlate.
...
As we can see in figure 9 the residuals visually look very similar. 
...
$$(y_{i1}, y_{i2}, ..., y_{iT- 2})$$. If there are strictly exogenous variables in our system $x_{it}$ that correlate with the endogenous variable and not with the residuals they should also be included in the weight matrix diagonals (for a detailed proof `r Citet(bib, 'dynamic.panel', .opts = list(cite.style = "numeric"))`):
...
model.dynamic <- suppressWarnings( pgmm(Total.Arrivals ~ Terror.attacks + hours.worked + tax + gdp + Investment,
                       lag.form=list(1, 0, 0, 0, 0, 0), gmm.inst = ~ Terror.attacks + hours.worked + tax + gdp + Investment, 
                       lag.gmm = list(c(2,99)),
                       effect="individual",
                       data=pdata))
```
...
After trying various laggs and taking into account the economic logic behind the model we get the results in table 9. The dynamic model shows that terror attacks has a significant influence to tourist arrivals. One additional terror attack would decrease the total number of tourist arrivals by 0.02 percent. Additionally, a positive increase in investment to infrastructure would lead to a decline
in tourist arrivals which is not logical according to the literature. Thus we will hold that this variable is not a significant one.

\newpage

\subsection{Dynamic models' residuals}

```{r residuals of dynamic models, echo=F, fig.cap="Residuals of models", fig.height=7, fig.width=8,  fig.pos="!ht", fig.align="center"}
...
```
As we can see in figure 11 the residuals resemble a simmilar pattern as in the case with static panel models. Altough the quantile to quantile plot shows "lighter" tails than in the previous cases. 

\subsection{Conclusion of panel data models' analysis}

Based on the data and using static random effects ant fixed effects panel models we can conlude that the only significant variable to tourist arrivals' changes is changes to the average hours worked in a given country. On average, a one percent increase in hours worked would lead to an increase of 1 percent of tourist arrivals. This can be explained that more hours worked would lead to more bussiness and hence an increase in tourist attractions in a given country. According to the dynamic model, terrorism is also an influential factor to changes in tourist arrivals. But according to that model it woul take about 50 additional attacks a year to lover tourist arrivals by 1 percent. Thus its influence is not very high.
This can be explainded that the services that prevent terrorism is doing a good job and the relative number of incidents are not that high to prevent tourists from visiting a country.
...
```{r histogram of structure models, echo=F, fig.cap="Histograms", fig.height=6, fig.width=8,  fig.pos="!ht", fig.align="center"}
...
As we can see from table 10, the only significant variables are GDP per capita abroad and hours worked domestically (with significance level of 0.1).
...
As we can see from figure 13 we can conclude that the residuals from the structure model graphically look the most normal out of all the models in this analysis so far.
...
We can clearly see from figure 14 that there are some observations which cook's distance is much larger than the generally accepted rule of 4 divided by the number of observations. We will eliminate the points from our data set who's residual is larger in absolute value than 2.5.
...
Eliminating the influential points and re-regressed the model we got that the variable hours is not significant after all. Also we can see that from figure 15 that the residuals are visually normal. The Shapiro-Wilk test's statistic is 0.039 which with alpha level of 0.01 would imply normality.
...
The final structure model is in table 13.
...
Based on random effects, fixed effects, dynamic panel models and linear models and on the public data available we can conclude that:

$\bullet$ Terror attacks have little or no impact to overall change to tourist arrivals in the countries which we have analyzed.
...
$\bullet$ Percentage change of investments to infrastructure in country i does not influence the rate of change in tourist arrivals.

$\bullet$ Percentage change of annual hours worked has no effect on the changes of tourism flows. 
...
$\bullet$ The weak correlation between the variables can be attributed to the big aggregation of tourism flows in the data that can be viewed publically. In order to have better results one can use commercial data (UNWTO database for example). 
...
The f test `r Citet(bib, 'F.test', .opts = list(cite.style = "numeric"), after=" , pp. 3-4")` that is used with the panel models in R statistical package tests the following hypothesis: assume that we have two models: 
...
The Hausman test `r Citet(bib, 'Hausman.test', .opts = list(cite.style = "numeric"), after=" , pp. 39-40")` is used to determine whether $X_{it}$ are uncorrelated with $\alpha_{i}$. To test this we denote two estimators:
