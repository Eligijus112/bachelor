...
The countries which will be analyzed are all members of the OECD (\textbf{Organisation for Economic Co-operation and Development}) [see @OECD]. Currently there are 35 member states but in this analysis we will be adding Lithuania and Costa Rica because it is strongly believed that these countries will soon join OECD.
...
\[
log(A)_{it}  = \textbf{f}(log(GDP)_{it}, terror_{it}, log(hours)_{it})
\]
...
If there is no heteroskedasticity in the residuals we should see a pattern where the spread of points is uniform in the y-direction as we move along the x-axis. Figure 13 shows that in the random effects model case this is not true - there is a downwards pattern. Hence, according to the residuals, the 'better' model is the fixed - effects model.
...
$$W_{i} = \left( \begin{pmatrix}
    [y_{i1}, x_{i1}, x_{i2}]       & 0 & \dots & 0 \\
    0       &  [y_{i1}, y_{i2} x_{i1}, x_{i2}, x_{i3}] & \dots & 0 \\
    0      & 0  & \dots & [y_{i1}, y_{i2}, ..., y_{iT-2}, x_{i1}, x_{i2}, x_{i3}, ..., x_{iT-1}]
\end{pmatrix} \right) $$

Then the unbiased \textit{Arellano and Bond} estimator is given by:




To determine the "best" lags we will be using the cross-validation method (MAPE statistic). We will split the data to a training set and test set like in the previous section. We will also assume that the coefficient near the terror attacs is negative, GDP.per.capita - positive and hours worked - negative. 
