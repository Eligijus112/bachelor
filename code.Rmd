...
As we can see from table 5 and the statistics for accuracy table the fixed effects models' predictions are closer to the original values. 
...
After trying various laggs and taking into account the economic logic behind the model we get the results in table 7. We see that we get a very similar result as in the static case.
...
We will save the p values and the coefficients near the explanatory variables and analyze the means and the distributions of them. 
...
$X_{itk}$ - both the domestic and abroad economy's GDP per capita, domestic terror attacks.
...
The X is a matrix of 5 collumns (intercept collumn and the economic variables) and has the same number of rows as Y.
...
model.ols <-  lm(Arrivals ~  gdp.a + gdp.d + terror + hours, data=sdt.sub)
